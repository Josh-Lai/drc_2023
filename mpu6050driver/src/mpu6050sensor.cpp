...
{
  uint16_t accel_x = i2c_smbus_read_word_data(file_, ACCEL_XOUT_H);
  double accel_x_converted = convertRawAccelerometerData(((accel_x >> 8) | (accel_x << 8)));
  if (calibrated_) {
...
{
  uint16_t accel_y = i2c_smbus_read_word_data(file_, ACCEL_YOUT_H);
  double accel_y_converted = convertRawAccelerometerData((accel_y >> 8) | (accel_y << 8));
  if (calibrated_) {
...
{
  uint16_t accel_z = i2c_smbus_read_word_data(file_, ACCEL_ZOUT_H);
  double accel_z_converted = convertRawAccelerometerData((accel_z >> 8) | (accel_z << 8));
  if (calibrated_) {
...
{
  uint16_t gyro_x = i2c_smbus_read_word_data(file_, GYRO_XOUT_H);
  double gyro_x_converted = convertRawGyroscopeData((gyro_x >> 8) | (gyro_x << 8));
  if (calibrated_) {
...
{
  uint16_t gyro_y= i2c_smbus_read_word_data(file_, GYRO_YOUT_H);
  double gyro_y_converted = convertRawGyroscopeData((gyro_y >> 8) | (gyro_y << 8));
  if (calibrated_) {
...
{
  int16_t gyro_z= i2c_smbus_read_word_data(file_, GYRO_ZOUT_H);
  double gyro_z_converted = convertRawGyroscopeData((gyro_z >> 8) | (gyro_z << 8));
  if (calibrated_) {
