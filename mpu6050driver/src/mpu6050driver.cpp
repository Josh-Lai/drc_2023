...
void MPU6050Driver::handleInput()
{
  ...
  message.linear_acceleration.y = mpu6050_->getAccelerationX();
  message.linear_acceleration.x = mpu6050_->getAccelerationY();
  message.linear_acceleration.z = -mpu6050_->getAccelerationZ();
  message.angular_velocity_covariance[0] = {0};
  message.angular_velocity.y = mpu6050_->getAngularVelocityX() * M_PI / 180;
  message.angular_velocity.x = mpu6050_->getAngularVelocityY() * M_PI / 180;
  message.angular_velocity.z = -mpu6050_->getAngularVelocityZ() * M_PI / 180;
